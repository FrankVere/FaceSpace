...
// this will handle the user sign in
const handleSignIn = (req, res) => {
  console.log(req.body);
  const { firstName } = req.body;
  const isSignedIn = res.locals.users.find((user) => {
    return user.name === firstName;
  });
  if (isSignedIn) {
    sendResponse(res, 200, isSignedIn, "User signed in");
  } else {
    sendResponse(res, 400, "Cannot find user");
  }
};
